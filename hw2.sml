...
fun score (lc, goal) =
	...
	case all_same_color(lc) of
	    true => prel(lc, goal) div 2
	  | false => prel(lc, goal)
    end

fun officiate (lc, ml, goal) =
    let
	val held = []
	fun aux (lc, [], goal, held) = score(held, goal)
	  | aux (lc, m :: ms, goal, held) =
	    case m of
		Discard card => aux(lc, ms, goal, remove_card(held, card, IllegalMove))
	      | Draw => case lc of
			    [] => score(held, goal) 
			   |c :: cs => case sum_cards(c :: held) > goal of
					    false => aux(cs, ms, goal, c :: held)
					  | true => score(c :: held, goal)  
    in
	aux(lc, ml, goal, [])
    end
